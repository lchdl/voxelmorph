...
parser.add_argument('--mask', required=True, help='mask for moving image')
parser.add_argument('--fixed', required=True, help='fixed image (target) filename')
parser.add_argument('--moved', required=True, help='warped image output filename')
parser.add_argument('--moved-mask', required=True, help='warped mask output filename')
...
mask = vxm.py.utils.load_volfile(args.mask, add_batch_axis=True, add_feat_axis=add_feat_axis)
...
if args.moved:
    ...
    moved_mask = moved_mask.detach().cpu().numpy().squeeze()
    vxm.py.utils.save_volfile(moved, args.moved, fixed_affine)
    vxm.py.utils.save_volfile(moved_mask, args.moved_mask, fixed_affine)
